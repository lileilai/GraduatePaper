...
\begin{table}[htpb]
  ...
  \end{tabular}
\end{table}
根据数据，以Recall为横坐标，Precision为纵坐标，可以画出如图\ref{fig:exp-map}所示的曲线c。将recall 值划分为11个值，我们将$Recall \geq \widetilde{r}$的Precision 替换为最大的，由此可得到曲线c。计算方法如公式
...
在预训练模型，包括ResNet-101和Vgg-16最后一层的维度都是是4096，经过全连接层，在消息传递和池化模块的中的编码长度为512。结合周边物体信息模块中注意力机制的attention\_size=30，在微调ResNet-101和Vgg-16，卷积层学习率设置为$1 \times 10^{-4}$，新增的分类层学习率为$1 \times 10^{-3}$。训练时，学习率设置为$1 \times 10^{-4}$，权重衰减为$5 \times 10^{-4}$。对于PISC\_coarse和PISC\_fine数据集，消息传递和池化模块的迭代数$T=4$，PIPA\_relation，迭代次数为$T=3$。 并且微调时的优化方法是SGD，训练迭代模块的优化方法是Adam。

\section{PISC数据集实验结果}
在实验中，本文主要与以下几个模型进行对照，在我们的整体框架中，主要的有特征提取模块（m0）、消息传递和池化模块（m1），周边物体信息模块（m2），对于m0和m1组成的网络为PPRN-obj（简称PPRN），而m0、m1和m2三个模块一起的模型称为PPRN+obj：
\begin{itemize}
...
在表\ref{tab:exp-result-1}中，我们展示了在PISC数据集上，按照每个类别的召回率和``mAP''的设定得到的实验结果，这里需要说明的是因为在PIS\_coarse中的关系粒度已经是最粗的，所以在实际实现过程中只存在关系损失，domain loss只在PISC\_fine 和PIPA数据集上进行了实验。
\begin{table*}[htpb]
  \centering
...
\begin{enumerate}
    ...
    \item 其次，结合表\ref{tab:exp-result-1}的数据，对于coarse-level的识别，PPRN取得了75.1\% 的准确率，81.8\%的mAP。对于fine-level的识别，PPRN-obj取得了65.6\%的准确率，69.7\% 的mAP。本文提出的模型在fine-level 的识别任务上超过了所有的基准模型。与此同时，PPRN+d(PPRN+domain loss)取得了66.2\%的准确率，72.0\%的mAP，证明了引入domain loss的作用。模型在coarse-level的识别任务上略微低于GRM，但是仍然超过了其它引入了周边物体区域的模型。
\end{enumerate}
...
\begin{enumerate}
    ...
    \item 本文通过引入的domain loss，在PISC\_fine上取得了明显的提升，模型通过同时优化relation loss和domain loss，这样两个任务之间可以共享一些信息，并且相互约束，帮助区分属于不同关系域的关系。
...
除此之外，为了观察PPRN+d(PPRN-obj+domain loss)在社会关系理解任务上的具体表现，我们在PISC-fine的测试集中随机抽取了部分图片，并且列出了他们的具体表现。在图\ref{fig:exp-case-study}中，我们分别展示了4个样例的结果，左边代表原图片，右边是社会关系图谱，其中(A)表示标注，(B)表示我们模型构建的社会关系图谱，(C)表示基准模型中GRM的结果。其中图片中人的包围盒的颜色对应社会关系图谱中节点的颜色，节点间不同颜色的边表示不同的社会关系，在图
